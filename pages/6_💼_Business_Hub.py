...
with t_wissen:
    ...
    with st.expander("⚖️ 1. Wann muss ich ein Gewerbe anmelden?"):
        st.write("Sobald eine **Gewinnerzielungsabsicht** vorliegt, musst du ein Gewerbe anmelden. Nutze als Anfänger die Kleinunternehmerregelung.")
    with st.expander("📧 2. Die professionelle Business-Mail"):
        st.write("Sichere dir eine eigene Domain (z.B. `deincreatorname.de`). Das erhöht deine Chancen auf Markenkooperationen drastisch!")
    with st.expander("🔗 3. Die 'Link-in-Bio'-Falle"):
        st.write("Viele US-Tools sind datenschutzrechtlich schwierig. Am sichersten ist eine eigene, kleine Website für deine Links.")

with t_links:
    st.subheader("🔗 Affiliate- & Partner-Links")
    with st.container(border=True):
